...
def seed():
    if not UserModel.query.first():
        db.session.add(UserModel(username="admin", password=generate_password_hash("admin123"), role="admin"))
        db.session.add(UserModel(username="staff", password=generate_password_hash("staff123"), role="staff"))
    ...
    if not TableModel.query.first():
        for n, c, loc in [(1,4,"Interior"),(2,2,"Terraza"),(3,6,"Interior"),(4,4,"Terraza"),(5,8,"Salón privado"),(6,2,"Barra")]:
...
def add_order_item(oid):
    ...
    try:
        ...
        # Si ya existe ese producto en la orden, sumar cantidad
...
def reports():
    ...
    # CODE SMELL: duplicated from calculate_revenue_stats — para el lab
    max_ticket = 0
    for b in all_paid:
